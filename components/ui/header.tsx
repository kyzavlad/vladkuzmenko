...
// cn не используется, можно убрать import { cn } from "@/lib/utils";

export function Header() {
  const [isMobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [isChoosePathOpen, setChoosePathOpen] = useState(false);

  // ... (ваш массив navigationItems без изменений) ...
  ...
  const handleNavClick = (
    ...
    e.preventDefault();
    setMobileMenuOpen(false); // Закрываем мобильное меню при любом клике навигации
    ...
    if (href.startsWith("#")) {
      ...
      window.location.href = href; // или router.push(href)
    }
  };
  ...
  return (
    <>
      ...
      {isChoosePathOpen && (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" // z-index для оверлея ниже модалки
      ...
      {isChoosePathOpen && (
        // ... (код вашего модального окна Choose Path без изменений, он выглядел корректным по ширине) ...
        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-lg px-4">
          <div className="bg-background/95 backdrop-blur-sm border border-border/40 rounded-2xl p-6 space-y-4">
            ...
            <div onClick={() => { window.open('https://vladkuzmenko.team', '_blank'); setChoosePathOpen(false); }} className="cursor-pointer">
              <StarBorder className="w-full hover:scale-[1.02] transition-transform" color="hsl(var(--color-3))" >
                <div className="flex items-center justify-between">
                  <div className="text-left">
      ...
      <header className="w-full z-30 fixed top-0 left-0 bg-background/95 backdrop-blur-sm border-b border-border/40">
        {/* ИЗМЕНЕНИЕ ДЛЯ КОНТЕЙНЕРА ХЕДЕРА:
          На мобильных (<lg): 'flex flex-row items-center justify-between'. Это расставит логотип и иконку меню по краям.
          На десктопах (lg+): 'lg:grid lg:grid-cols-[auto_1fr_auto] lg:gap-x-4'. 
          Логотип в центральной колонке (1fr) будет занимать доступное место и центрироваться текстом, 
          боковые колонки (auto) будут по ширине содержимого.
        */}
        <div className="container relative mx-auto py-4 md:py-5 flex flex-row items-center justify-between lg:grid lg:grid-cols-[auto_1fr_auto] lg:gap-x-4">
          
          {/* Left side (Desktop navigation) */}
          <div className="hidden lg:flex justify-start items-center"> {/* Было lg:col-span-1, для [auto_1fr_auto] это не нужно явно */}
            {/* ... ваш код десктопной навигации ... (оставлен без изменений) */}
            <NavigationMenu>
                  ...
                  <NavigationMenuItem key={item.title}>
                    {item.href ? (
                      <NavigationMenuLink asChild>
                        <a href={item.href} onClick={(e) => handleNavClick(e, item.href!)} className="inline-flex h-10 items-center justify-center whitespace-nowrap rounded-md px-3 py-2 text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus:outline-none disabled:pointer-events-none disabled:opacity-50" >
                          {item.title}
                        </a>
                      </NavigationMenuLink>
                    ) : (
                      <>
                        <NavigationMenuTrigger className="h-10 px-3 py-2 font-medium text-sm"> {item.title} </NavigationMenuTrigger>
                            ...
                            <div className="flex flex-col h-full justify-between">
                              ...
                              <div>
                                <ContactDialog triggerText="Book a call today">
                                  <Button size="sm" className="mt-6 w-full md:w-auto"> Book a call today </Button>
                                </ContactDialog>
                              </div>
                            </div>
                            <div className="flex flex-col text-sm gap-1">
                              {item.items?.map((subItem) => (
                                <a href={subItem.href} key={subItem.title} onClick={(e) => handleNavClick(e, subItem.href)} className="flex flex-row justify-between items-center hover:bg-accent hover:text-accent-foreground py-2 px-3 rounded-md transition-colors" >
          ...
          {/* Center (Logo) */}
          {/* ИЗМЕНЕНИЯ ДЛЯ ЛОГОТИПА:
            - Обертка теперь 'flex justify-center items-center min-w-0'. min-w-0 важно, чтобы колонка могла сжиматься.
            - У '.logo-container' нет Tailwind класса 'overflow-hidden' (управляется из globals.css).
            - У текста логотипа нет 'whitespace-nowrap'.
          */}
          <div className="flex justify-center items-center min-w-0"> {/* min-w-0 позволяет центральной колонке сжиматься */}
            <div className="logo-container relative"> {/* Класс из globals.css */}
              <a href="/" className="flex items-center" aria-label="VladKuzmenko.com Home">
                <span className="text-xl sm:text-2xl font-bold tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-foreground via-foreground to-foreground/30 dark:from-white dark:via-white dark:to-white/30 font-serif italic">
                  VladKuzmenko.com
                </span>
              </a>
            </div>
          ...
          <div className="flex justify-end items-center"> {/* Общий контейнер для правых элементов */}
        ...
        {isMobileMenuOpen && (
          // ... (код вашего мобильного выпадающего меню без изменений, он выглядел корректным по ширине) ...
