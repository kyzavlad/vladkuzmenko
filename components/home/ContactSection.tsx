...
const COPY: Record<Lang, {
  ...
  en: {
    eyebrow: "Next step",
    titleA: "Something is stuck? ",
    titleB: "Start with the real bottleneck.",
    desc: "Tell me what you are trying to grow, launch or simplify and where it is breaking down today. You do not need to choose the technology first — the useful next step comes from the problem.",
    callTitle: "Book a focused call",
    callDesc: "Bring the real situation. We will look at the constraint, what has already been tried and where the first useful intervention should be.",
    formTitle: "Send the situation directly",
  },
  ua: {
    eyebrow: "Наступний крок",
    titleA: "Щось застрягло? ",
    titleB: "Почнімо з реального вузького місця.",
    desc: "Опишіть, що хочете виростити, запустити або спростити й де це ламається сьогодні. Не потрібно спочатку обирати технологію — корисний наступний крок визначає сама проблема.",
    callTitle: "Записатися на предметний дзвінок",
    callDesc: "Принесіть реальну ситуацію. Розберемо обмеження, що вже пробували, і де має бути перше корисне втручання.",
    formTitle: "Надіслати ситуацію напряму",
  },
  ru: {
    eyebrow: "Следующий шаг",
    titleA: "Что-то не двигается? ",
    titleB: "Начнём с реального узкого места.",
    desc: "Опишите, что хотите вырастить, запустить или упростить и где это ломается сегодня. Не нужно заранее выбирать технологию — полезный следующий шаг определяется самой проблемой.",
    callTitle: "Записаться на предметный звонок",
    callDesc: "Принесите реальную ситуацию. Разберём ограничение, что уже пробовали и где должно быть первое полезное вмешательство.",
    formTitle: "Отправить ситуацию напрямую",
  },
};
...
export function ContactSection() {
  ...
  return (
    <section id="contact" className="relative scroll-mt-24 overflow-hidden bg-[#020202] py-24 md:py-32">
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        <div className="absolute left-1/2 top-0 h-[620px] w-[94%] -translate-x-1/2 bg-[radial-gradient(ellipse_at_top,rgba(212,175,55,.09),rgba(255,255,255,.012)_38%,transparent_70%)]" />
        <div className="absolute left-1/2 top-12 h-px w-[min(800px,78vw)] -translate-x-1/2 bg-gradient-to-r from-transparent via-amber-300/34 to-transparent shadow-[0_0_30px_rgba(212,175,55,.13)]" />
      </div>
        ...
        <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-[.85fr_1.15fr]">
          <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col rounded-[32px] border border-white/[.09] bg-[linear-gradient(145deg,rgba(255,255,255,.04),rgba(255,255,255,.012))] p-6 sm:p-8">
            ...
            <a href={SITE.calcom} target="_blank" rel="noopener noreferrer" className="mt-7">
              <Button className="premium-button h-auto min-h-12 w-full px-6 py-3.5">{x.callTitle}<ArrowRight className="ml-2 h-4 w-4" /></Button>
            </a>

            <div className="mt-7 border-t border-white/[.07] pt-5">
              <div className="grid gap-2.5">
                <a href={SITE.socials.telegram} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 rounded-2xl border border-white/[.07] bg-black/28 px-4 py-3 text-sm text-zinc-400 transition-colors hover:border-white/[.14] hover:text-white"><Send className="h-4 w-4 text-sky-300/75" />Telegram</a>
                <a href={SITE.socials.whatsapp} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 rounded-2xl border border-white/[.07] bg-black/28 px-4 py-3 text-sm text-zinc-400 transition-colors hover:border-white/[.14] hover:text-white"><MessageCircle className="h-4 w-4 text-emerald-300/75" />WhatsApp</a>
                <a href={`mailto:${SITE.email}`} className="flex items-center gap-3 rounded-2xl border border-white/[.07] bg-black/28 px-4 py-3 text-sm text-zinc-400 transition-colors hover:border-white/[.14] hover:text-white"><Mail className="h-4 w-4 text-amber-300/75" />{SITE.email}</a>
              </div>
            </div>
          </motion.div>
