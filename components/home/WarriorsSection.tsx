...
export function WarriorsSection() {
  ...
  return (
    <section
      id="warriors"
      className="relative scroll-mt-24 overflow-hidden border-t border-white/[.07] bg-[#030203] py-24 md:py-32"
    >
        ...
        <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[.92fr_1.08fr] lg:gap-14">
          ...
          >
            ...
            <h2 className="section-title mt-4 text-[clamp(2.8rem,5vw,4.8rem)] text-zinc-100">
              {j.titleA}
              <em className="bg-gradient-to-br from-violet-100 via-violet-300 to-amber-200 bg-clip-text font-normal italic text-transparent">{j.titleB}</em>
            </h2>
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              {x.principles.slice(0, 4).map((principle) => (
                <div key={principle} className="flex gap-3 rounded-2xl border border-white/[.07] bg-white/[.018] p-4">
                  <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-violet-300/20 bg-violet-300/[.06] text-violet-200">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              <a href={href} className="w-full sm:w-auto" onClick={() => track("warriors_open", { source: "home" })}>
                <Button className="h-auto min-h-12 w-full border border-violet-300/20 bg-violet-300/[.05] px-7 py-3 text-white hover:bg-violet-300/[.09] sm:w-auto">
                  {x.open}
                </Button>
              </a>
            </div>
          ...
          >
            <InteractiveSurface
              accent="violet"
              lift={false}
              className="relative overflow-hidden rounded-[30px] border border-violet-200/[.12] bg-[linear-gradient(145deg,rgba(196,181,253,.055),rgba(255,255,255,.016)_48%,rgba(0,0,0,.48))] p-6 shadow-[0_42px_120px_-60px_rgba(139,92,246,.18)] sm:p-8"
            >
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-zinc-100">Warriors Team</p>
                  <p className="mt-1 text-[10px] uppercase tracking-[.17em] text-zinc-600">{j.pathLabel}</p>
                </div>
                <span className="flex h-10 w-10 items-center justify-center rounded-xl border border-violet-300/20 bg-violet-300/[.06] text-violet-200">
                  <Network className="h-4 w-4" />
                </span>
              </div>

              <div className="relative mt-7">
                <div aria-hidden="true" className="absolute bottom-6 left-[19px] top-6 w-px bg-gradient-to-b from-amber-300/45 via-violet-300/35 to-violet-300/0" />
                  ...
                  {j.steps.map((step, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="relative grid grid-cols-[40px_1fr] gap-4 rounded-2xl border border-white/[.07] bg-black/26 p-4"
                      >
                        <span className={`relative z-10 flex h-10 w-10 items-center justify-center rounded-xl border ${index === 0 ? "border-amber-300/25 bg-amber-300/[.07] text-amber-200" : "border-violet-300/18 bg-violet-300/[.045] text-violet-200/80"}`}>
