...
import { ArrowRight, BriefcaseBusiness, CheckCircle2, Dumbbell, Search, ShieldCheck, Sparkles, UtensilsCrossed } from "lucide-react";
...
const ICONS = [BriefcaseBusiness, Dumbbell, UtensilsCrossed, CheckCircle2];
            ...
            {x.business.flatMap((label, index) => {
              const node = (
                <motion.div
                  ...
                  className="relative rounded-2xl border border-white/[.08] bg-white/[.025] px-3 py-5 text-center"
                >
              ...
              return [
                node,
                <div key={`line-${label}`} className="relative h-px min-w-5 overflow-hidden bg-white/[.08]">
                  {!reduced && (
                    <motion.span
                      className="absolute inset-y-0 left-0 w-1/2 bg-gradient-to-r from-transparent via-amber-300 to-transparent"
                      animate={{ x: ["-120%", "220%"] }}
                      transition={{ duration: 2.4, repeat: Infinity, ease: "linear", delay: index * .35 }}
                    />
                  )}
                </div>,
              ];
    ...
    return (
      <div className="relative min-h-[280px] overflow-hidden rounded-[26px] border border-sky-300/12 bg-[#040607] p-6 sm:p-8">
        {!reduced && (
          <motion.div
            aria-hidden="true"
            className="absolute inset-x-5 top-5 z-20 h-px bg-gradient-to-r from-transparent via-sky-300/90 to-transparent shadow-[0_0_24px_rgba(125,211,252,.5)]"
            animate={{ y: [0, 225, 0] }}
          ...
          {x.visibility.map((label, index) => {
            ...
            return (
              <motion.div
                ...
                className="min-h-[105px] rounded-2xl border border-white/[.075] bg-white/[.022] p-4"
              >
                ...
                <div className="mt-3 flex gap-1">
                  {[0, 1, 2, 3].map((bar) => (
                    <span key={bar} className="h-1 flex-1 rounded-full bg-sky-300/35" />
                  ))}
                </div>
              </motion.div>
            );
        ...
        <svg viewBox="0 0 530 280" className="absolute inset-0 h-full w-full" aria-hidden="true">
          {lines.map(([a,b], index) => (
            <motion.line
              ...
              stroke="rgba(196,181,253,.24)" strokeWidth="1.2" strokeDasharray="4 7"
              initial={reduced ? false : { pathLength: 0, opacity: 0 }}
              whileInView={{ pathLength: 1, opacity: 1 }} viewport={{ once: true }}
              transition={{ duration: .8, delay: index * .06 }}
            />
          ))}
          {nodes.map(([cx,cy], index) => (
            <motion.circle
              key={index} cx={cx} cy={cy} r={index === 0 ? 9 : 6}
              fill={index === 0 ? "rgba(245,190,52,.95)" : "rgba(196,181,253,.85)"}
              initial={reduced ? false : { scale: 0 }} whileInView={{ scale: 1 }} viewport={{ once: true }}
              transition={{ type: "spring", delay: .15 + index * .07 }}
            />
          ))}
        </svg>
        <div className="absolute inset-x-6 bottom-5 rounded-2xl border border-white/[.07] bg-black/55 px-4 py-3 backdrop-blur-xl">
          <p className="text-center text-[10px] font-medium tracking-[.04em] text-zinc-400">{x.network}</p>
        </div>
      </div>
    );
  }
        ...
        {x.performance.map((label, index) => {
          ...
          return (
            <motion.div
              ...
              className="flex min-h-[150px] flex-col items-center justify-center rounded-2xl border border-white/[.07] bg-black/45 px-3 text-center"
            >
              <span className={`flex h-10 w-10 items-center justify-center rounded-xl border ${index === 2 ? "border-amber-300/24 bg-amber-300/[.06] text-amber-200" : "border-emerald-300/18 bg-emerald-300/[.045] text-emerald-200/80"}`}>
