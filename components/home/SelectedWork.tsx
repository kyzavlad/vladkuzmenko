...
const COPY = {
  en: {
    ...
    title: "Proof in real products",
    desc: "A few systems shown end to end: the problem, what was built and the product that exists today.",
    task: "Task", built: "What was built", result: "Result", capabilities: "Key features",
    open: "Open project", live: "Open live site", all: "View all work", noVisual: "Project preview",
  },
  ua: {
    eyebrow: "Вибрані роботи",
    title: "Доказ у реальних продуктах",
    desc: "Кілька систем повністю: задача, що було побудовано і продукт, який існує сьогодні.",
    task: "Завдання", built: "Що зроблено", result: "Результат", capabilities: "Ключові можливості",
    open: "Відкрити проєкт", live: "Відкрити живий сайт", all: "Переглянути всі роботи", noVisual: "Прев’ю проєкту",
  },
  ru: {
    eyebrow: "Избранные работы",
    title: "Доказательство в реальных продуктах",
    desc: "Несколько систем целиком: задача, что было построено и продукт, который существует сегодня.",
    task: "Задача", built: "Что сделано", result: "Результат", capabilities: "Ключевые возможности",
    open: "Открыть проект", live: "Открыть живой сайт", all: "Смотреть все работы", noVisual: "Превью проекта",
  },
} as const;
      ...
      <div className="grid lg:grid-cols-[.9fr_1.1fr]">
        ...
        <div className="p-5 sm:p-7 lg:p-8">
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <span className="rounded-full border border-white/10 bg-white/[.035] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[.16em] text-amber-200/80">{CATEGORY_SHORT[p.category][locale]}</span>
            <span className="ml-auto hidden text-[10px] font-semibold uppercase tracking-[.16em] text-zinc-700 sm:block">{String(i + 1).padStart(2, "0")}</span>
          </div>
          ...
          {c.value && (
            <div className="mt-4 rounded-2xl border border-amber-300/15 bg-[linear-gradient(135deg,rgba(245,190,52,.07),rgba(255,255,255,.015))] p-4">
              <div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.18em] text-amber-300/80"><ShieldCheck className="h-3.5 w-3.5" />{x.result}</div>
