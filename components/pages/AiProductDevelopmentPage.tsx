...
import { STATUS_LABEL, STATUS_TONE, type CaseStatus } from "@/lib/case-studies";
import { PRODUCT_CASES, EXPERIENCE_MATRIX, APD_UI } from "@/lib/portfolio";
...
function StatusBadge({ status, lang }: { status: CaseStatus; lang: "en" | "ua" | "ru" }) {
  return (
    <span
      className={cn(
    ...
    <div className="bg-black text-white min-h-screen">
      ...
      <section className="section-tint relative pt-36 pb-16 border-b border-zinc-900">
        <div className="container mx-auto px-4 max-w-4xl relative z-10">
          ...
          <p className="text-sm text-gray-500 leading-relaxed max-w-3xl mb-8">
            {ui.leadIntro}
          </p>
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="space-y-8">
            {PRODUCT_CASES.map((pc) => {
              ...
              const caseHref = pc.caseSlug ? `${localeBase}/work/${pc.caseSlug}` : undefined;
              return (
                <article key={pc.key} className="luxe-card p-6 sm:p-8">
                  ...
                  {caseHref && (
                    <a
                      href={caseHref}
                      className="mt-6 inline-flex items-center gap-1.5 text-sm font-medium text-amber-300 hover:text-amber-200"
                    >
                      {ui.viewCase}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {EXPERIENCE_MATRIX.map((row, i) => (
                    <tr
                      key={row.capability.en}
                      className={cn(i % 2 === 1 && "bg-white/[0.02]", "align-top")}
                    >
