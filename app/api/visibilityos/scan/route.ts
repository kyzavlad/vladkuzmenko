...
const USER_AGENT = "VisibilityOS/2.1 (+https://vladkuzmenko.com/visibilityos)";
...
const ACTION_COPY: Record<Lang, Record<string, { title: string; reason: string; action: string }>> = {
  en: {
    ...
    schema: { title: "Clarify the local business entity", reason: "Local business structured data was not detected in the sampled pages.", action: "Add truthful LocalBusiness or ProfessionalService structured data only when it matches the visible business and service-area information." },
    faq: { title: "Turn repeated buying questions into crawlable answers", reason: "No clear FAQ evidence was detected in the sampled pages.", action: "Add concise, real FAQs around buying objections and service scope where they genuinely help users." },
    trustPages: { title: "Strengthen trust and business identity", reason: "The sampled journey could not confirm both an About path and a direct Contact path.", action: "Make the business identity, contact route and relevant proof easy to find from the main journey." },
  ...
  ua: {
    ...
    schema: { title: "Уточніть локальну business entity", reason: "На вибірці сторінок не знайдено local-business structured data.", action: "Додавайте правдиві LocalBusiness або ProfessionalService structured data лише коли вони відповідають видимій інформації про бізнес і зону роботи." },
    faq: { title: "Перетворіть повторювані питання покупців на crawlable answers", reason: "На вибірці сторінок не знайдено чітких FAQ-сигналів.", action: "Додайте реальні короткі FAQ про заперечення та scope послуги там, де це допомагає користувачу." },
    trustPages: { title: "Посильте довіру та ідентичність бізнесу", reason: "Скан не зміг підтвердити одночасно About path і прямий Contact path.", action: "Зробіть інформацію про бізнес, контактний шлях і релевантні докази легкодоступними з основного journey." },
  ...
  ru: {
    ...
    schema: { title: "Уточните локальную business entity", reason: "На выборке страниц не обнаружены local-business structured data.", action: "Добавляйте правдивые LocalBusiness или ProfessionalService structured data только когда они соответствуют видимой информации о бизнесе и зоне работы." },
    faq: { title: "Превратите повторяющиеся вопросы покупателей в crawlable answers", reason: "На выборке страниц не обнаружено явных FAQ-сигналов.", action: "Добавьте реальные короткие FAQ про возражения и scope услуги там, где это помогает пользователю." },
    trustPages: { title: "Усильте доверие и идентичность бизнеса", reason: "Скан не смог подтвердить одновременно About path и прямой Contact path.", action: "Сделайте информацию о бизнесе, контактный путь и релевантные доказательства легко доступными из основного journey." },
...
const CONTEXT_STOP_WORDS = new Set([
  "and", "the", "for", "with", "from", "your", "our", "service", "services", "solutions",
  "та", "для", "про", "послуга", "послуги", "рішення",
  "и", "для", "про", "услуга", "услуги", "решения",
]);
...
function normalizeTextInput(value: unknown, maxLength: number): string {
...
async function assertPublicHost(url: URL): Promise<void> {
  ...
  if (hostname === "localhost" || hostname.endsWith(".localhost") || hostname.endsWith(".local") || hostname.endsWith(".internal") || hostname.endsWith(".home")) {
  ...
  for (let hop = 0; hop <= MAX_REDIRECTS; hop += 1) {
    ...
    try {
      response = await fetch(current, {
        ...
        cache: "no-store",
        headers: {
          Accept: mode === "html" ? "text/html,application/xhtml+xml;q=0.9,*/*;q=0.5" : "text/plain,application/xml,text/xml,*/*;q=0.5",
    ...
    if (mode === "html" && contentType && !contentType.includes("text/html") && !contentType.includes("application/xhtml+xml")) {
...
function normalizeSearchText(value: string): string {
  return value.toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/[^\p{L}\p{N}]+/gu, " ").replace(/\s+/g, " ").trim();
}

function keywordTokens(value: string): string[] {
  return [...new Set(normalizeSearchText(value).split(" ").filter((token) => token.length >= 2 && !CONTEXT_STOP_WORDS.has(token)))].slice(0, 10);
}

function containsContext(haystack: string, context: string): boolean {
  const tokens = keywordTokens(context);
  if (!tokens.length) return false;
  const normalized = ` ${normalizeSearchText(haystack)} `;
  const matches = tokens.filter((token) => normalized.includes(` ${token} `) || normalized.includes(token)).length;
  const required = tokens.length <= 2 ? tokens.length : Math.max(2, Math.ceil(tokens.length * 0.5));
  return matches >= required;
}

function isBroadLocationContext(value: string): boolean {
  const normalized = normalizeSearchText(value);
  if (!normalized) return false;
  return /^(worldwide|global|remote|online|international|everywhere|all countries|anywhere|по всему миру|весь мир|глобально|онлайн|удаленно|удалённо|міжнародно|по всьому світу|весь світ|глобально|онлайн|віддалено)$/i.test(normalized);
}

function inferPageKind(url: string, title: string, h1: string[], service: string, location: string): ScanPageKind {
  let path = "";
  try { path = decodeURIComponent(new URL(url).pathname).toLowerCase(); } catch { /* no-op */ }
  const surface = `${path.replace(/[-_/]/g, " ")} ${title} ${h1.join(" ")}`;
...
function extractInternalCandidates(html: string, baseUrl: string): string[] {
  const candidates = new Set<string>();
  for (const match of html.matchAll(/<a\b[^>]+href=["']([^"']+)["']/gi)) {
    const candidate = cleanCandidate(match[1], baseUrl);
    if (candidate) candidates.add(candidate);
    if (candidates.size >= MAX_DISCOVERED_LINKS) break;
  }
  return [...candidates];
}
...
function extractSitemapUrls(xml: string, baseUrl: string): string[] {
  ...
  for (const match of xml.matchAll(/<loc>\s*([^<]+)\s*<\/loc>/gi)) {
    const candidate = cleanCandidate(match[1].replace(/&amp;/g, "&"), baseUrl);
    if (candidate) urls.add(candidate);
    if (urls.size >= MAX_DISCOVERED_LINKS) break;
  }
  return [...urls];
}

function hasAboutPath(html: string): boolean {
  return [...html.matchAll(/<a\b[^>]+href=["']([^"']+)["']/gi)].some((match) =>
    /(?:^|\/|#)(about|about-us|company|o-nas|pro-nas|про-нас|о-нас)(?:\/|#|$)/i.test(match[1]),
  );
}

function hasContactPath(html: string): boolean {
  return [...html.matchAll(/<a\b[^>]+href=["']([^"']+)["']/gi)].some((match) =>
    /^(mailto:|tel:)|(?:wa\.me|api\.whatsapp\.com|cal\.com|t\.me)|(?:^|\/|#)(contact|contacts|kontakt|контакт|контакты|контакти|book|booking|call|demo|audit|request)(?:\/|#|$)/i.test(match[1]),
  );
}

function candidateScore(urlValue: string, service: string, location: string): number {
  try {
    const path = decodeURIComponent(new URL(urlValue).pathname).toLowerCase();
    const pathText = path.replace(/[-_/]/g, " ");
    let score = 0;
    if (service && containsContext(pathText, service)) score += 60;
    if (location && containsContext(pathText, location)) score += 55;
    if (/\/(services?|uslugi|poslug|послуг|услуг)/i.test(path)) score += 35;
    if (/\/(locations?|areas?|cities|mista|goroda|міст|город)/i.test(path)) score += 32;
    if (/\/(contact|contacts|kontakt|контакт)/i.test(path)) score += 24;
    if (/\/(about|about-us|company|o-nas|pro-nas|про-нас|о-нас)/i.test(path)) score += 20;
    if (/\/(faq|questions|pytann|vopros)/i.test(path)) score += 18;
...
function pageSummary(input: { url: string; statusCode: number; durationMs: number; html: string; service: string; location: string }): ScanPageSummary {
  ...
  let path = "/";
  try { path = new URL(input.url).pathname || "/"; } catch { /* no-op */ }
  return {
    url: input.url,
    path,
    kind: inferPageKind(input.url, signals.title, signals.h1, input.service, input.location),
    ...
    lang: signals.lang,
    forms: signals.forms,
    contactLinks: signals.contactLinks,
    internalLinks: signals.internalLinks,
    externalLinks: signals.externalLinks,
    schemaTypes: signals.schemaTypes,
    serviceMatch: input.service ? containsContext(searchable, input.service) : false,
...
function scoreFindings(findings: VisibilitySiteScanResult["findings"], categories: ScanCategory[]): number {
  return average(findings.filter((finding) => categories.includes(finding.category)).map((finding) => finding.status === "pass" ? 100 : finding.status === "warn" ? 55 : 0));
}

function calculatePillars(input: {
  base: ReturnType<typeof analyzeVisibilityScan>;
  ...
  const { base, pages, service, location, robotsFound, sitemapFound, paths } = input;
  const serviceCoverage = service ? pages.filter((page) => page.serviceMatch).length : 0;
  const locationCoverage = location ? pages.filter((page) => page.locationMatch).length : 0;
  const schemaTypes = pages.flatMap((page) => page.schemaTypes.map((type) => type.toLowerCase()));
  const hasEntitySchema = schemaTypes.some((type) => ["organization", "localbusiness", "professionalservice", "service"].includes(type));
  ...
  const trust = Math.min(100, Math.round(scoreFindings(base.findings, ["trust"]) * 0.65 + (hasEntitySchema ? 15 : 0) + (paths.aboutPathFound ? 10 : 0) + (paths.contactPathFound ? 10 : 0)));
  let local: number | null = null;
  if (location) {
    const servicePart = service ? Math.min(40, serviceCoverage * 20) : 20;
    const locationPart = Math.min(40, locationCoverage * 20);
    const schemaPart = schemaTypes.some((type) => ["localbusiness", "professionalservice"].includes(type)) ? 20 : 5;
    local = Math.min(100, servicePart + locationPart + schemaPart);
  }
...
function action(
  ...
  const copy = ACTION_COPY[lang][id] || ACTION_COPY.en[id];
  return { id, lane, category, impact, confidence, effort, title: copy.title, reason: copy.reason, evidence, action: copy.action };
}

function buildGrowthQueue(input: {
  lang: Lang;
  base: ReturnType<typeof analyzeVisibilityScan>;
  pages: ScanPageSummary[];
  service: string;
  ...
  const { lang, base, pages, service, location, robotsFound, sitemapFound, competitors, paths } = input;
  const actions: GrowthAction[] = [];
  const siteWideContactPaths = pages.reduce((sum, page) => sum + page.contactLinks, 0);
  const serviceCoveragePages = service ? pages.filter((page) => page.serviceMatch).length : 0;
  const locationCoveragePages = location ? pages.filter((page) => page.locationMatch).length : 0;
  const servicePageCandidates = pages.filter((page) => page.kind === "service").length;
  const locationPageCandidates = pages.filter((page) => page.kind === "location").length;
  const schemaTypes = pages.flatMap((page) => page.schemaTypes.map((type) => type.toLowerCase()));
  const localSchema = schemaTypes.some((type) => ["localbusiness", "professionalservice"].includes(type));
  const faqFound = pages.some((page) => page.kind === "faq" || page.schemaTypes.some((type) => type.toLowerCase() === "faqpage"));

  if (base.findings.some((finding) => finding.priority === "high" && finding.status !== "pass")) {
    const evidence = base.findings.filter((finding) => finding.priority === "high" && finding.status !== "pass").slice(0, 3).map((finding) => finding.title).join(", ");
    actions.push(action(lang, "searchBasics", "now", "seo", "high", "high", "low", evidence));
  }
  if (siteWideContactPaths < 2 && !paths.contactPathFound) actions.push(action(lang, "conversion", "now", "conversion", "high", "high", "low", `${siteWideContactPaths} direct contact/booking paths across ${pages.length} sampled pages`));
  ...
  if (location && !localSchema) actions.push(action(lang, "schema", "next", "local", "medium", "high", "medium", schemaTypes.length ? `Detected schema: ${[...new Set(schemaTypes)].join(", ")}` : "No local business entity schema detected"));
  ...
  if (!paths.aboutPathFound || !paths.contactPathFound) actions.push(action(lang, "trustPages", "later", "trust", "medium", "high", "medium", `About path: ${paths.aboutPathFound ? "found" : "not found"} · Contact path: ${paths.contactPathFound ? "found" : "not found"}`));
  const strongerCompetitor = competitors.find((competitor) => competitor.score >= base.score + 10);
  if (strongerCompetitor) actions.push(action(lang, "competitor", "next", "visibility", "medium", "medium", "medium", `${strongerCompetitor.resolvedUrl} observable score ${strongerCompetitor.score} vs ${base.score}`));

  const laneOrder: Record<GrowthLane, number> = { now: 0, next: 1, later: 2 };
  const impactOrder: Record<GrowthImpact, number> = { high: 0, medium: 1, low: 2 };
  const effortOrder: Record<GrowthEffort, number> = { low: 0, medium: 1, high: 2 };
...
async function tryFetchText(url: URL): Promise<{ found: boolean; url: string; text: string }> {
  try {
    const fetched = await fetchPublicResource(url, "text");
    return { found: fetched.response.ok, url: fetched.resolvedUrl, text: fetched.text };
  } catch {
    return { found: false, url: url.toString(), text: "" };
  }
}
...
async function scanCompetitor(value: string, lang: Lang): Promise<CompetitorSnapshot | null> {
  ...
  if (!url) return null;
  try {
    const fetched = await fetchPublicResource(url, "html");
    const scan = analyzeVisibilityScan({ requestedUrl: url.toString(), resolvedUrl: fetched.resolvedUrl, statusCode: fetched.response.status, durationMs: fetched.durationMs, headers: fetched.response.headers, html: fetched.text, lang });
    return {
      requestedUrl: url.toString(),
      resolvedUrl: scan.resolvedUrl,
      title: scan.signals.title,
      score: scan.score,
      summary: scan.summary,
      signals: { h1: scan.signals.h1, forms: scan.signals.forms, contactLinks: scan.signals.contactLinks, schemaTypes: scan.signals.schemaTypes, faqEvidence: scan.signals.faqEvidence },
    };
  } catch {
    return null;
  }
...
async function runScan(input: ScanInput) {
  ...
  const service = normalizeTextInput(input.service, 120);
  const requestedLocation = normalizeTextInput(input.location, 120);
  const localLocation = isBroadLocationContext(requestedLocation) ? "" : requestedLocation;
  const competitorInputs = Array.isArray(input.competitors)
    ? input.competitors.filter((value): value is string => typeof value === "string").map((value) => value.trim()).filter(Boolean).slice(0, MAX_COMPETITORS)
    : [];
  ...
  try {
    ...
    const robotsCandidate = new URL("/robots.txt", root);
    const defaultSitemapCandidate = new URL("/sitemap.xml", root);
    const [robots, defaultSitemap, competitorsSettled] = await Promise.all([
      tryFetchText(robotsCandidate),
      tryFetchText(defaultSitemapCandidate),
      Promise.all(competitorInputs.map((candidate) => scanCompetitor(candidate, lang))),
    ]);

    const declaredSitemap = robots.text.match(/^\s*Sitemap:\s*(\S+)/im)?.[1];
    let sitemap = defaultSitemap;
    if (declaredSitemap) {
      const declaredUrl = normalizeUrl(declaredSitemap);
      if (declaredUrl && normalizeComparableHost(declaredUrl.hostname) === normalizeComparableHost(root.hostname) && declaredUrl.toString() !== defaultSitemap.url) {
        sitemap = await tryFetchText(declaredUrl);
      }
    }

    const discovered = new Set<string>();
    const homeUrl = cleanCandidate(home.resolvedUrl, home.resolvedUrl) || home.resolvedUrl;
    discovered.add(homeUrl);
    for (const candidate of extractInternalCandidates(home.text, home.resolvedUrl)) discovered.add(candidate);
    if (sitemap.found) for (const candidate of extractSitemapUrls(sitemap.text, home.resolvedUrl)) discovered.add(candidate);

    const rankedCandidates = [...discovered]
      .filter((candidate) => candidate !== homeUrl)
      .sort((a, b) => candidateScore(b, service, localLocation) - candidateScore(a, service, localLocation))
      .slice(0, MAX_SITE_PAGES - 1);

    const pageFetches = await Promise.allSettled(rankedCandidates.map(async (candidate) => {
      const fetched = await fetchPublicResource(new URL(candidate), "html");
      return pageSummary({ url: fetched.resolvedUrl, statusCode: fetched.response.status, durationMs: fetched.durationMs, html: fetched.text, service, location: localLocation });
    }));

    const pages: ScanPageSummary[] = [
      pageSummary({ url: home.resolvedUrl, statusCode: home.response.status, durationMs: home.durationMs, html: home.text, service, location: localLocation }),
      ...pageFetches.filter((result): result is PromiseFulfilledResult<ScanPageSummary> => result.status === "fulfilled").map((result) => result.value),
    ];
    const competitors = competitorsSettled.filter((item): item is CompetitorSnapshot => Boolean(item));
    const paths: QueueContext = {
      aboutPathFound: pages.some((page) => page.kind === "about") || hasAboutPath(home.text),
      contactPathFound: pages.some((page) => page.kind === "contact") || hasContactPath(home.text) || base.signals.contactLinks > 0,
    };
    const pillars = calculatePillars({ base, pages, service, location: localLocation, robotsFound: robots.found, sitemapFound: sitemap.found, paths });
    const schemaTypes = pages.flatMap((page) => page.schemaTypes.map((type) => type.toLowerCase()));
    const serviceCoveragePages = service ? pages.filter((page) => page.serviceMatch).length : 0;
    const locationCoveragePages = localLocation ? pages.filter((page) => page.locationMatch).length : 0;
    const site = {
      ...
      serviceCoveragePages,
      locationCoveragePages,
      servicePageCandidates: pages.filter((page) => page.kind === "service").length,
      locationPageCandidates: localLocation ? pages.filter((page) => page.kind === "location").length : 0,
      contactPageFound: paths.contactPathFound,
      aboutPageFound: paths.aboutPathFound,
      faqFound: pages.some((page) => page.kind === "faq" || page.schemaTypes.some((type) => type.toLowerCase() === "faqpage")),
      localBusinessSchemaFound: schemaTypes.some((type) => ["localbusiness", "professionalservice"].includes(type)),
      organizationSchemaFound: schemaTypes.some((type) => type === "organization"),
      siteWideContactPaths: pages.reduce((sum, page) => sum + page.contactLinks, 0),
      siteWideForms: pages.reduce((sum, page) => sum + page.forms, 0),
    };
    const growthQueue = buildGrowthQueue({ lang, base, pages, service, location: localLocation, robotsFound: robots.found, sitemapFound: sitemap.found, competitors, paths });
    const pillarValues = [pillars.search, pillars.conversion, pillars.trust, ...(pillars.local === null ? [] : [pillars.local])];
    const siteScore = average(pillarValues);
    const result: VisibilitySiteScanResult = {
      ...base,
      version: 2,
      score: siteScore,
      context: { service, location: requestedLocation },
      pillars,
      site,
      pages,
      growthQueue,
      competitors,
      limitations: getVisibilityScanLimitations(lang),
    };
    return NextResponse.json({ ok: true, result }, { headers: { "Cache-Control": "no-store", "X-VisibilityOS-Version": "2.1" } });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  let body: unknown;
  try { body = await request.json(); } catch { return NextResponse.json({ ok: false, error: ERROR_COPY.en.invalid }, { status: 400 }); }
  const parsed = body && typeof body === "object" ? body as ScanInput : {};
  return runScan(parsed);
}

export async function GET(request: NextRequest) {
  const competitors = request.nextUrl.searchParams.getAll("competitor");
  return runScan({
